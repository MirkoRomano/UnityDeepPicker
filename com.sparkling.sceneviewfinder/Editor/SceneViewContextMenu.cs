...
public class SceneViewContextMenu : PopupWindowContent
{
    ...
    private IEnumerable<UnityEngine.Object> objects;
    private IEnumerable<UnityEngine.Object> filteredObjects;
    ...
    public SceneViewContextMenu(IEnumerable<UnityEngine.Object> objs)
    {
    ...
    private void FilterObjects()
    {
        ...
        if (searchFilter.StartsWith(TYPE_SEARCH_KEYWORD, StringComparison.OrdinalIgnoreCase))
        {
            ...
            typesCache.TryGetValue(filterType, out Type searchType);
            filteredObjects = objects.Where(o =>
            {
                ...
                if (searchType != null)
                {
                    return o.GetComponent(searchType) != null;
                }
                ...
                foreach (Component c in o.GetComponents<Component>())
                {
                    ...
                    typesCache.TryAdd(componentType.Name, componentType);

                    if (string.Equals(filterType, componentType.Name, StringComparison.OrdinalIgnoreCase))
                    {
        ...
        else if (searchFilter.StartsWith(LABEL_SEARCH_KEYWORD, StringComparison.OrdinalIgnoreCase))
        {
            ...
            filteredObjects = objects.Where(o =>
            {
                ...
                foreach (string label in AssetDatabase.GetLabels(o))
                {
        ...
        else if (searchFilter.StartsWith(TAG_SEARCH_KEYWORD, StringComparison.OrdinalIgnoreCase))
        {
            ...
            filteredObjects = objects.Where(o =>
            {
                ...
                try
                {
                    ...
                    {
                        return string.Equals(obj.tag, targetTag);
                    }
                    else
                    {
                        return false;
        ...
        {
            filteredObjects = objects.Where(o => o.name.Contains(searchFilter, StringComparison.OrdinalIgnoreCase)).OrderBy(o => o.name);
        }
    }
}
